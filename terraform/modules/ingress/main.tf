...
resource "helm_release" "aws_load_balancer_controller" {
  ...
  depends_on = [kubernetes_namespace.ingress, aws_iam_role.load_balancer_controller]
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(var.cluster_oidc_issuer_url, "https://", "")}:sub" = "system:serviceaccount:${kubernetes_namespace.ingress.metadata[0].name}:aws-load-balancer-controller"
          }
        }
      }
    ]
